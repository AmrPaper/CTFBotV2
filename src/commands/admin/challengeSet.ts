...
export async function browseLibrary(msg: Message): Promise<void> {
    try {
        ...
        console.error(`browseLibrary operation failed`, error);
    }
}
...
export async function selectChallengeSet(msg: Message, args: string[]): Promise<void> {
    try {
        ...
        console.error(`selectChallengeSet function failed`, error);
    }
}

export async function grabChallengeSet(): Promise<ChallengeSet | null> {
    try {
        ...
        if (!challengeContent) {
            throw new Error(`Failed to load challenge set from: ${challengeSetPath}`);
        }
        return challengeContent;
    } catch (error) {
        console.error(`grab challenge function failed and will return a null value`, error);
        return null;
    }
}